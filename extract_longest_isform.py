...
def get_transcript_dic(trinity_fasta):
	with open (trinity_fasta,'r') as trinity_fa:
		transcript={}
		transcript_length=[]
	...
	return transcript_length,transcript
...
def extract_longest_isform(transcript,name):
	...
	for i_key,i_value in gene_dit.items():
			id = re.sub('>','',i_value[2])
			f1.write(id+'\t'+str(i_value[1])+'\n')
			unigene_length.append(i_value[1])
	f1.close()
...
if __name__ == '__main__':
    try:
        trinity_fa=sys.argv[1]
        name=sys.argv[2]
    except IndexError:
        print("Usage: python " + sys.argv[0] + " Trinity.fasta name\n")
        sys.exit(1)
    if trinity_fa == None or name == None:
        print("Usage: python "+ sys.argv[0]+" Trinity.fasta name\n")
    else:
        transcript_length,transcript = get_transcript_dic(trinity_fa)
